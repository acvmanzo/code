...
def dictagnum(kind, fname, allorex):
    """Generates a dictionary where the keywords are genotypes and the values 
...
def plottitle(metric, kind):
    ...
    if metric == 'agnummean' or metric=='agnummedall' or metric == 'agnummedex':
        if kind == 'wingthreat':
            t = 'Number of\nwing threats'
        if kind == 'minag':
...
def loadplotdata(metric, kind, fname, ctrlkey, binconf, keyfile):
    ...
    if metric == 'agnummedall':
        d = dictagnum(kind, fname, 'all')
        md = dictmw(d, ctrlkey)
        adjpd = mcpval(md, 'fdr', 'True', keyfile)

    if metric == 'agnummedex':
        d = dictagnum(kind, fname, 'ex')
        md = dictmw(d, ctrlkey)
        adjpd = mcpval(md, 'fdr', 'True', keyfile)

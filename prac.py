...
cmn.makenewdir(DIR)
createinfolat(OFILELAT)
createinfoprop(OFILEPROP)
...
for k in ks:
    multiplot_1bar(k[0], FNAME, BARNUM, BARWIDTH, XLIM, YMIN, YLABEL1, yaxisticks=YAXISTICKS1, subplotn=k[1], subplotl=k[2], fontsz=FONTSZ, stitlesz=STITLESZ, lw=LW)

    d =cl.dictfreq(k[0], FNAME)
    writeinfolat(FNAME, OFILELAT, k[0])
    writeinfoprop(FNAME, OFILEPROP, k[0])

multiplot_3bars(KINDLIST, FNAME, 'keylist', conf=0.05, ylabel=YLABEL2, yaxisticks=YAXISTICKS2, ymin=YMIN2, ylim=YLIM2, colors=COLORS, subplotn=SUBPLOTNS[-1], subplotl=SUBPLOTLS[-1], barwidth=BARWIDTH, barnum=BARNUM, fontsz=FONTSZ, stitlesz=STITLESZ, leglabels=LEGLABELS, lw=LW)
